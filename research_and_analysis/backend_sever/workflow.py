...
from research_and_analysis.backend_sever.models import (
    Analyst,
    Perspectives,
...
from research_and_analysis.prompt_library.prompts import *
...
def build_interview_graph(llm,tavily_search=None):
    ...
    def search_web(state: InterviewState):
        ...
        """
        messages = state["messages"]
        search_query = messages[-1].content if messages else state["analyst"].description

        try:
            structure_llm = llm.with_structured_output(SearchQuery)
            structured_query = structure_llm.invoke(
                [SystemMessage(content=GENERATE_SEARCH_QUERY)] + messages
            )
            if structured_query.search_query:
                search_query = structured_query.search_query
        except Exception:
            pass
        ...
        formatted_search_docs = "\n\n---\n\n".join(
            [
                ...
                f'{doc.get("content", doc.get("raw_content", ""))}\n</Document>'
...
class AutonomousReportGenerator:
    def __init__(self, llm):
        ...
        self.tavily_search = TavilySearch(tavily_api_key=TAVILY_API_KEY)
    ...
    def human_feedback(self):
        """
        Collect feedback from the human user.
        """
        pass
    ...
    def write_report(self, state: ResearchGraphState):
        ...
        if not sections:
            sections = ["No sections generated - please verify interview process."]

        report = self.llm.invoke([
            SystemMessage(content=REPORT_WRITER_INSTRUCTIONS.format(topic=topic)),
    ...
    def write_introduction(self,state:ResearchGraphState):
        ...
        intro = self.llm.invoke([SystemMessage(content=instructions)]+[HumanMessage(content="Write the report introduction")])
    ...
    def write_conclusion(self,state:ResearchGraphState):
        ...
        instructions = INTRO_CONCLUSION_INSTRUCTIONS.format(topic=topic, formatted_str_sections=formatted_str_sections)
        conclusion = self.llm.invoke([SystemMessage(content=instructions)]+[HumanMessage(content="Write the report conclusion")])
